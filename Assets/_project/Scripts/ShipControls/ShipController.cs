...
public class ShipController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            blaster.Init(this,WeaponInput, _shipData.BlasterCooldown, 0, _shipData.BlasterProjectileDuration, _shipData.BlasterDamage, _rigidBody);
        }

        foreach (MissileLauncher launcher in _missileLaunchers)
        {
            launcher.Init(this,WeaponInput);
        }
    ...
    void FixedUpdate()
    {
        if (_movementControls is DesktopMovementControls && ! _rigidBody.isKinematic)
        {
    ...
    public void ShakeShip(float strengthFactor)
    {
        if (Camera.main)
        {
            Tween.ShakeCamera(Camera.main, strengthFactor: strengthFactor);
        }
    }
}
